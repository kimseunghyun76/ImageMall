        ...
        <form class="form-horizontal" name="userfrm" id="userfrm" onsubmit="return onSubmitcheck();" method="post">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <select id="user_role" name="user_role" class="input-xlarge">
                        <c:if test="${my_user_role =='3' || userinfo.user_role == '3'}">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    ...
                    <c:if test="${userinfo.user_id != my_user_id}">
...
<script>
    function onSubmitcheck(){
        ...
        if($("#password").val() !="" && $("#password").val().length < 8) {
            ...
            return false;
        }else if($("#password").val() !="" && !$("#password").val().match(/([a-zA-Z0-9].*[!,@,#,$,%,^,&,*,?,_,~])|([!,@,#,$,%,^,&,*,?,_,~].*[a-zA-Z0-9])/)) {
        ...
        if($("#shop_name").val() =="" || $("#shop_name").val().length < 2) {
            ...
            <c:if test="${my_user_role == '1'}">
            document.userfrm.action="/user/edit2?${_csrf.parameterName}=${_csrf.token}";
            </c:if>

            <c:if test="${my_user_role != '1'}">
            document.userfrm.action="/admin/edit2?${_csrf.parameterName}=${_csrf.token}";
            </c:if>
            return true;
        }
        return false;
